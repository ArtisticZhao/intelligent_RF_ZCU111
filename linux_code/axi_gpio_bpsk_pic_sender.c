...
int main()
{
	...
	*(volatile uint32_t*)(bpsk_virt_addr) = 0x01;  // power on
	...
	// get bmp file
	FILE *fp = NULL;
	fp = fopen("/home/root/ov_out.bmp", "rb");
	char res;
	uint8_t buf[148];
	res = get_kiss_frame(fp, buf, 148);
	uint8_t *bufp=buf;
	// write ram1
	wr_cnt = 0;
	for(int i=0; i<38*4; i+=4){
		if(wr_cnt==0){
			...
			*(volatile uint32_t*)(bram_virt_addr + 0x200 + i) = empty_frame[wr_cnt]^reversebytes_uint32t(*(uint32_t*)bufp);
			bufp+=4;
		}
		wr_cnt++;
	}
	...
	for(int i=0; i<38*4; i+=4){
		if(wr_cnt==0){
			...
			*(volatile uint32_t*)(bram_virt_addr + 0x100 + i) = empty_frame[wr_cnt]^0xc0c0c0c0;
			bufp+=4;
		}
		wr_cnt++;
	}
	*(volatile uint32_t*)(bpsk_virt_addr + 8) = 0x01;  // ram1 data ready.
	// ping pang send
	uint32_t gpio_val=0;
	while(1){
	...
	close(fd);
	fclose(fp);
	*(volatile uint32_t*)(bpsk_virt_addr + 4) = 0x00;
	*(volatile uint32_t*)(bpsk_virt_addr + 8) = 0x00;
	sleep(5);
